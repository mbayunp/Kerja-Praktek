...
from controlers.codecorr_answer_controler import(
    add_answer,
    retrieve_answers,
    retrieve_answer,
)
...
from models.codecorr_answer import (
    CodeCorrAnswerSchema,
)

router = APIRouter()

@router.post("/", response_description="Answer data added into the database")
async def add_answer_data(answer: CodeCorrAnswerSchema = Body(...)):
